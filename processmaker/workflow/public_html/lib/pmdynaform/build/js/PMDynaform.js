...
(function(){
	var GridView = PMDynaform.view.Field.extend({
		...
		addRow: function (e) {
            ...
            if ( e === undefined && rowData && (this.gridtable.length < this.model.get("rows"))) {
		...
		*/
		...
		createHTMLFields: function (numberRow) {
			...
			for (k=0; k < this.gridtable[numberRow].length; k+=1) {
				...
				//} else {
					this.gridtable[numberRow][k].model.set("name",this.changeNameField(this.model.get("name"),numberRow+1,this.gridtable[numberRow][k].model.get("columnName")));
					this.gridtable[numberRow][k].render();
					element = this.gridtable[numberRow][k].el;
...
(function(){
	var DatetimeView = PMDynaform.view.Field.extend({
		...
		render : function (isConsole){
			var data = {}, date, that = this, clickEvent, hidden, name;
			if (!isConsole){
	            this.$el.html( this.template(this.model.toJSON()) );
	            if (this.model.get("hint") !== "") {
	                this.enableTooltip();
